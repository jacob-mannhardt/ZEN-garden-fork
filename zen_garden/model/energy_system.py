...
class EnergySystem:
    ...
    def store_input_data(self):
        ...
        self.carbon_emissions_annual_limit_nodal = self.data_input.extract_input_data(
            "carbon_emissions_annual_limit_nodal", 
        ...
        self.carbon_emissions_annual_limit_nodal = (
            self.carbon_emissions_annual_limit_nodal * _fraction_year
        )  # reduce to fraction of year
    ...
    def construct_params(self):
        ...
        # carbon emissions limit
        parameters.add_parameter(
            name="carbon_emissions_annual_limit_nodal", 
            set_time_steps="set_time_steps_yearly",
            index_names=["set_nodes","set_time_steps_yearly"], 
            doc='Parameter which specifies the total limit on carbon emissions', 
            calling_class=cls
        )
    ...
    def construct_vars(self):
        ...
        variables.add_variable(
            model, 
            name="carbon_emissions_annual_nodal", 
            index_sets=Element.create_custom_set(["set_nodes", "set_time_steps_yearly"], 
                                                 self.optimization_setup), 
            doc="annual carbon emissions of energy system for each node", 
            unit_category={"emissions": 1})
...
class EnergySystemRules(GenericRule):
    ...
    def constraint_carbon_emissions_annual_limit_nodal(self):
        r""" time dependent carbon emissions limit from 
        technologies and carriers for each node
        ...
        self.constraints.add_constraint(
            "constraint_carbon_emissions_annual_limit_nodal",constraints
        )
    ...
    def constraint_carbon_emissions_annual_nodal(self):
        r""" add up all carbon emissions from technologies and carriers for each node

        .. math::
            E_{y,n} = E_{y,n,\\mathcal{H}} + E_{y,n,\\mathcal{C}}

        :math:`E_{y,n,\\mathcal{H}}`: carbon emissions from technologies 
        in year :math:`y` at node :math:`n` \n
        :math:`E_{y,n,\\mathcal{C}}`: carbon emissions from carriers 
        in year :math:`y` at node :math:`n`
        """
        term_carbon_emissions_carrier = (
                self.variables["carbon_emissions_carrier"] 
                * self.get_year_time_step_duration_array()).sum(
        ...
        term_carbon_emissions_technology = term_carbon_emissions_technology.rename(
            {"set_location": "set_nodes"})
        # add flow terms
        ### index sets
        if ("carbon" in self.sets["set_carriers"] 
            and self.variables["flow_transport"].size > 0):
            ...
            edges_in = {(node,edge):1 for node in self.sets["set_nodes"] 
                for edge in self.energy_system.calculate_connected_edges(node, "in")}
            edges_out = {(node,edge):1 for node in self.sets["set_nodes"] 
                for edge in self.energy_system.calculate_connected_edges(node, "out")}
            ...
            term_flow_transport_in = (
                self.variables["flow_transport"] * edges_in
                ).sum(["set_edges"])
            term_flow_transport_out = (
                self.variables["flow_transport"] * edges_out
                ).sum(["set_edges"])
            term_flow_transport_loss = (
                self.variables["flow_transport_loss"] * edges_in
                ).sum(["set_edges"])
            term_net_flow = (
                - term_flow_transport_in + 
                term_flow_transport_out + 
                term_flow_transport_loss)
            carbon_techs = [tech for tech in 
                            self.sets["set_transport_technologies"] 
                            if "carbon" in self.sets["set_reference_carriers"][tech]]
            term_net_flow_carbon = term_net_flow.sel(
                {"set_transport_technologies": carbon_techs}
                ).sum(["set_transport_technologies"])
            assert "carbon_storage" in self.sets["set_conversion_technologies"], \
            ("The set of conversion technologies must contain "
            "'carbon_storage' to calculate carbon storage.")
            carbon_intensity_carbon_storage = (
                self.parameters.carbon_intensity_technology.sel(
                    {"set_technologies":"carbon_storage"}).to_series().dropna().iloc[0]
            )
            term_net_flow_carbon = (term_net_flow_carbon * 
                                    xr.DataArray(carbon_intensity_carbon_storage))
            time_step_duration = self.get_year_time_step_duration_array()
            term_net_flow_carbon = (
                term_net_flow_carbon.assign_coords(time_step_duration.coords) * 
                time_step_duration).sum("set_time_steps_operation")
            term_net_flow_carbon = term_net_flow_carbon.reindex_like(
                term_carbon_emissions_technology.const)
        else:
            # if there is no carrier flow we just create empty arrays
            term_net_flow_carbon = (
                term_carbon_emissions_technology.where(False).to_linexpr())
        # sum up all terms
        # build LHS
        ...
        constraints = lhs == rhs
        self.constraints.add_constraint(
            "constraint_carbon_emissions_annual_nodal",constraints)
