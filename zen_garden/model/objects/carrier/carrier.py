...
class Carrier(Element):
    ...
    def construct_params(cls, optimization_setup):
        ...
        # transport limit into the node
        optimization_setup.parameters.add_parameter(name="transport_limit_in", index_names=["set_carriers", "set_nodes", "set_time_steps_yearly"], doc='Parameter which specifies the transport limit into the node', calling_class=cls)
        # transport limit out of the node
        optimization_setup.parameters.add_parameter(name="transport_limit_out", index_names=["set_carriers", "set_nodes", "set_time_steps_yearly"], doc='Parameter which specifies the transport limit out of the node', calling_class=cls)
    ...
    def construct_constraints(cls, optimization_setup):
        ...
        # limit import/export flow by transport limit
...
class CarrierRules(GenericRule):
    ...
    def constraint_transport_limit(self):
        ...
        .. math::
            \\sum_{t\\in\\mathcal{T}}\\tau_t\\sum_{j\\in\\mathcal{J}}\\sum_{e\\in\\underline{\\mathcal{E}}}(F_{j,e,t}-F^\\mathrm{l}_{j,e,t}) \\leq b^\\mathrm{in}_{c,n,y} \n
            \\sum_{t\\in\\mathcal{T}}\\tau_t\\sum_{j\\in\\mathcal{J}}\\sum_{e'\\in\\overline{\\mathcal{E}}}F_{j,e',t}) \\leq b^\\mathrm{out}_{c,n,y}
        ...
        :math:`b^\\mathrm{in}_{c,n,y}`: transport limit into the node :math:`n` of carrier :math:`c` in year :math:`y` \n
        :math:`b^\\mathrm{out}_{c,n,y}`: transport limit out of the node :math:`n` of carrier :math:`c` in year :math:`y`\n
        """
        ### index sets
