...
class StorageTechnologyRules(GenericRule):
    ...
    def constraint_storage_technology_capex(self):
        ...
        lhs = linexpr_from_tuple_np([(1.0, self.variables["cost_capex"].loc[techs, capacity_types, nodes, times]),
                                     ...
                                     (-self.parameters.capex_specific_storage.loc[techs, capacity_types, nodes, times]*self.parameters.capex_share_scrapping, self.variables["capacity_addition_scrapped"].loc[techs, capacity_types, nodes, times])
                                     ],
                                     coords, self.model)
        rhs = 0
        constraints = lhs == rhs
